...
The longest-running shows card reads SOLT's own list on its data-and-research page, the productions table (the musicals table repeats three of its top four and is not read), with the month SOLT says the counts were supplied as the period. A running show's count is a floor and the value keeps SOLT's "Over"; a list whose counts do not fall rank by rank is refused.
