...
- **Spent facts.** A fact already posted with the same label and value leaves the pool, and any pair group with a spent member goes with it. A live vein's values change every run, so it is never spent; a monthly vein's facts are spent for the rest of the month. For the three yearly ticket-sales veins (`west_end`, `london_cinema`, `west_end_shows`) a spent fact expires after 180 days (`SPENT_EXPIRY_DAYS`), so each of their cards can post at most twice a year rather than once; a history record whose timestamp cannot be read still counts as spent. If nothing fresh is left, the slot is skipped, and the log says so.
- **Cooldowns.** Any vein that led a card within 20 hours is withheld, two groups of near-static veins are withheld for four days after leading, and `west_end_shows`, whose four cuts of one list would otherwise go out on consecutive days, for three weeks. A cooldown that would leave nothing to post is not applied.
...
The longest-running shows card reads SOLT's own list on its data-and-research page, the productions table (the musicals table repeats three of its top four and is not read), with the month SOLT says the counts were supplied as the period. A running show's count is a floor and the value keeps SOLT's "Over"; a list whose counts do not fall rank by rank is refused. The four cuts each carry their own label form, since a fact's id is its vein and label and the spent guard withholds a whole pair when one row is spent; the year cuts put the opening year in the value column and mark a running show by SOLT's own floor.
